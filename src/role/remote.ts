    ...
    depositHarvester: (data: RemoteHarvesterData): ICreepConfig => ({
        ...
        target: creep => {
            ...
            if(room.terminal){
                ...
                // 转移并检测返回值
                const result = creep.transfer(room.storage, creep.memory.depositType)
                if (result === OK || result === ERR_NOT_ENOUGH_RESOURCES) {
                    // 获取旗帜，旗帜没了就自杀
                    const targetFlag = Game.flags[data.sourceFlagName]
                    if (!targetFlag) {
                ...
                if (result === ERR_NOT_IN_RANGE) creep.farMoveTo(room.storage.pos, 1)
                else creep.say(`转移 ${result}`)
            }
