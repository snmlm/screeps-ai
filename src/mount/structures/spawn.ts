import { ROOM_TRANSFER_TASK, creepDefaultMemory, bodyConfigs, importantRoles,bodyConfigs1,bodyCost,generateBody } from 'setting'
...
export default class SpawnExtension extends StructureSpawn {
    ...
    private getBodys(bodyType: BodyAutoConfigConstant): BodyPartConstant[] {
        ...
        if(bodyType == 'upgrader'){
            if(this.room.controller.level == 8){
                return generateBody(this.room.energyAvailable,
                    bodyConfigs1.upgrader.rcl8.pattern,
                    bodyConfigs1.upgrader.rcl8.sizeLimit)   
            }else if(this.room.energyCapacityAvailable < 400){
                return generateBody(this.room.energyAvailable,
                    bodyConfigs1.upgrader.rcl2.pattern,1)
            }else{
                return generateBody(this.room.energyAvailable,
                    bodyConfigs1.upgrader.default.pattern,
                    bodyConfigs1.upgrader.default.sizeLimit)
            }
    ...
    private bodyCost(bodyparts: BodyPartConstant[]): number {
