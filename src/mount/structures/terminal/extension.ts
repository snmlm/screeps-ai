...
export default class TerminalExtension extends StructureTerminal {
        ...
        if (this.room.extractor) {
            ...
            for(var r in resc){
                if(resc[r] == this.room.mineral.mineralType){
                    ...
                    this.addTask(this.room.mineral.mineralType, 20000, terminalModes.put, terminalChannels.release)
                }else{
            ...
            case terminalChannels.share:
                if (resource.mod === terminalModes.get) this.room.shareRequest(resource.type, resource.amount - resourceAmount)
                    ...
                    if(resourceAmount <= resource.amount){
                        this.room.shareRemoveSource(resource.type)
                    }else{
                        this.room.shareAddSource(resource.type)
                    }
                }
                return this.setNextIndex()
            break
    ...
    private getOrderPrice(resourceType: ResourceConstant, type: ORDER_BUY | ORDER_SELL): number | undefined {
        ...
        const cachePriceCheck = global.resourcePriceCheck[`${resourceType}/${type}`]
        if(cachePriceCheck){
            global.resourcePriceCheck[`${resourceType}/${type}`] = cachePriceCheck + 1;
        }else{
            global.resourcePriceCheck[`${resourceType}/${type}`] = 1;
        }
        //console.log(`${resourceType}/${type}:`+global.resourcePriceCheck[`${resourceType}/${type}`]+`    `+JSON.stringify(global.resourcePriceCheck))
        if (cachePrice && cachePriceCheck < 10) {
            return cachePrice
        }
        global.resourcePriceCheck[`${resourceType}/${type}`] = 1;
        ...
        if (type === ORDER_SELL) {
            ...
            //     this.log(`无法为 ${resourceType} ${type} 创建订单，未找到市场其他订单`, 'yellow', true)
            // }else{
            //     if(Game.rooms[sortedOrders[0].roomName].controller.my){
            //         price = sortedOrders[1].price - 0.1;
            //     }else{
            //         price = sortedOrders[0].price - 0.1; 
            //     }
            // }
