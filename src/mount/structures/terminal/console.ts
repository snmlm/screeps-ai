...
export default class TerminalConsole extends TerminalExtension {
    ...
    public reset(hard: boolean = false): string {
        ...
        defaultResource.forEach(res =>{
            if(res == this.room.mineral.mineralType){
                this.add(res, 10000, terminalModes.put, terminalChannels.share)
                this.add(res, 20000, terminalModes.put, terminalChannels.release)
            }else{
                this.add(res, 1000, terminalModes.get, terminalChannels.share)
            }
        });
        return `已重置，当前监听任务如下:\n${this.show()}`
    }
