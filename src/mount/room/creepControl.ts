...
export default class CreepControl extends RoomConsole {
    ...
    public spawnRangedAttackerx(bearTowerNum: 0 | 1 | 3 | 5 | 2 | 4 | 6 = 6, targetFlagName: string = DEFAULT_FLAG_NAME.ATTACK, keepSpawn: boolean = false): string {

        const creepName = `${this.name} apocalypsex ${Game.time}`
        creepApi.add(creepName, 'apocalypsex', {
            targetFlagName: targetFlagName || DEFAULT_FLAG_NAME.ATTACK,
            bearTowerNum,
            keepSpawn
        }, this.name)
     ...
     * @param keepSpawn 是否持续生成
     */
    public spawnDismantleGroupL(targetFlagName: string = '', keepSpawn: boolean = false): string {
        const dismantlerName = `${this.name} dismantler ${Game.time}`
        const healerName = `${this.name} doctor ${Game.time}`
        creepApi.add(dismantlerName, 'dismantler', {
            targetFlagName: targetFlagName || DEFAULT_FLAG_NAME.ATTACK,
            healerName,
            keepSpawn
        }, this.name)
        creepApi.add(healerName, 'doctor', {
            creepName: dismantlerName,
            keepSpawn
        }, this.name)

        return `已发布拆墙小组无boost，正在孵化，GoodLuck Commander`
    }
     ...
     * @param num 要孵化的数量
     */
    public spwanSoldier(targetFlagName: string = '', num: number = 1,keepSpawn: boolean = false) {
     ...
     * 孵化基础治疗单位
     * 
     * @param targetFlagName 移动地点
     * @param num 要孵化的数量
     */
    public spwanDoctor(targetFlagName: string = '', num: number = 1) {
        if (num <=0 || num > 10) num = 1
        for (let i = 0; i < num; i++) {
            creepApi.add(`${this.name} doctorx ${Game.time}-${i}`, 'doctorx', {
                creepName: `${this.name} doctorx ${Game.time}-${i}`,
                keepSpawn: false,
                targetFlagName: targetFlagName
            }, this.name)
        }

        return `已发布 doctorx*${num}，正在孵化`
    }
