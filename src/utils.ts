...
export function stateScanner(): void {
    ...
    for (var roomName in Game.rooms){
        ...
        if (room.controller && room.controller.my) {
            const roomStr = `<a href="#!/room/${Game.shard.name}/${room.name}">${room.name}</a>`;
            let logLine1 = `${Game.time} ${roomStr}(${room.controller.level})`
            if (room.controller.progressTotal) {
                logLine1 += `${lpadround(room.controller.progress / room.controller.progressTotal*100, 3, 5)} %`;
                logLine1 += `  ${room.controller.progress}/${room.controller.progressTotal} `
            }
            logLine1 +=  ` - ${room.energyAvailable}/${room.energyCapacityAvailable}`;
            console.log(logLine1);
            if(room.storage){
                let logLine = `${Game.time} storge:`
                logLine += ` ${lpadround(room.storage.store.getUsedCapacity() / room.storage.store.getCapacity() * 100, 3, 5)} %`;
                for (let type in room.storage.store){
                    logLine += ` ${type}:${room.storage.store.getUsedCapacity(<ResourceConstant>type)}`
                }
                console.log(logLine);
            }
            if(room.terminal){
                let logLine = `${Game.time} terminal:`
                logLine += ` ${lpadround(room.terminal.store.getUsedCapacity() / room.terminal.store.getCapacity() * 100, 3, 5)} %`;
                for (var type in room.terminal.store){
                    logLine += ` ${type}:${room.terminal.store.getUsedCapacity(<ResourceConstant>type)}`
                }
                console.log(logLine);
            }
        }
    }
    console.log(`${Game.time} GCL ${Game.gcl.level}: ${lpadround(Game.gcl.progress/Game.gcl.progressTotal*100, 3, 5)} %  ${Math.round(Game.gcl.progress)}/${Math.round(Game.gcl.progressTotal)}`);
    console.log(`${Game.time} GPL ${Game.gpl.level}: ${lpadround(Game.gpl.progress/Game.gpl.progressTotal*100, 3, 5)} %  ${Math.round(Game.gpl.progress)}/${Math.round(Game.gpl.progressTotal)}`);
...
export function lpadround(nr,lpad,digest) {
